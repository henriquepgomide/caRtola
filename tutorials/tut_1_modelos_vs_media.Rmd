...
No tutorial anterior, vimos que a média é um indicador limitado para mitar no cartola. Vamos tentar neste tutorial, usar outros modelos e reduzir o risco na tomada de decisão. Começaremos de forma básica, com um modelo de regressão.

<center>![Levy Fidélix - Vamos subir o nível disso aqui](http://geradormemes.com/media/created/n931gu.jpg)</center>

Este tutorial também é dedicado a _iniciantes_ do R. 
...
require(devtools) # Carregar função para descrever pacotes e descrição do R
require(ggplot2)  # Plotar gráficos
require(ggthemes) # Carregar tema adicional para os gráficos
require(dplyr)    # Funções para manipulação dos dados 
require(caret)    # Pacote para modelar os dados
```


### Banco de dados

Usaremos os dados do [Cartola](https://github.com/henriquepgomide/caRtola/tree/master/db). Assim como no primeiro tutorial, iremos segmentar nosso banco em dois. 
...
variaveis <- c("atletas.clube.id.full.name", "atletas.jogos_num", "casa", "atletas.posicao_id", "goals_dif", "atletas.pontos_num")
