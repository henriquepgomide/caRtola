...
matches_fb <- subset(matches_fb, matches_fb$date <= "2017-09-18")
...
#%%%%%%%%%%%%%%%%%%%%%%%%%
df_pred <- subset(cartola, cartola$ano == 2017 & cartola$Rodada == 23 & cartola$Status == "Provável")
variaveis <- c(2, 3, 5, 7, 8, 9, 29,30, 32:70, 73:77)
df_pred <- df_pred[, variaveis]

df_pred$Rodada <- 24
df_pred <- left_join(x = df_pred[, -c(46:52)],
                     y = subset(temp2, temp2$round == 24 & temp2$ano == 2017), 
